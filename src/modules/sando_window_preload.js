...
let sammiPayload = getArgValue("sammipayload", process.argv);
const sammiVar = getArgValue("sammivar", process.argv);

if (sammiPayload !== "undefined" || !sammiPayload) {
...
ipc.on("SandoTriggerListener", (e, eventName, payload) => {
  if (!sammiId || !events[eventName] || events[eventName].length === 0) return;
...
contextBridge.exposeInMainWorld("Sando", {
  events: events,
  ...
  on: (eventName, callback) => {
    ...
    // ipc.send("SandoRegisterEvent", sammiId, eventName);
    events[eventName] ??= []; //nullish assignment operator, may not work in node versions under 15
    events[eventName].push(callback);
  },

  triggerExt: (extTrigger, params = {}) => {
  ...
  getVariable: async (name, button) => {
    const result = await getVariable(button, name);
    return result;
  },
  setVariable: (name, value, button, instance) => {
    if (!button) {
...
// ipc.on("SandoTriggerListener", (e, func, payload) => {
//   if (
//     !sammiId ||
...
function getVariable(targetButton, targetVar) {
  ...
  const windowHash = createWindowHash(sammiBtn, sammiInstance, sammiVar);
...
function createWindowHash(btn, instance, variable) {
  return MD5("" + btn + instance + variable);
}
